...
import com.alibaba.csp.sentinel.dashboard.rule.nacos.api.ApiDefinitionNacosProvider;
import com.alibaba.csp.sentinel.dashboard.rule.nacos.api.ApiDefinitionNacosPublisher;
...
public class GatewayApiController {
    ...
    @Autowired
    private ApiDefinitionNacosProvider provider;
    ...
    public Result<ApiDefinitionEntity> addApi(HttpServletRequest request, @RequestBody AddApiReqVo reqVo) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(allApis) && allApis.stream().map(o -> o.getApiName()).anyMatch(o -> o.equals(apiName.trim()))) {
...
//    }
    ...
    private boolean publishApis(String app, String ip, Integer port){
        ...
        try{
            ...
            logger.error("publish gateway apis error:", e);
            return false;
        }
    }
}
