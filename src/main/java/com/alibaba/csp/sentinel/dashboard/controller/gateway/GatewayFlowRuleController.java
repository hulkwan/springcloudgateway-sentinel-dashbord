...
import com.alibaba.csp.sentinel.dashboard.rule.nacos.gatewayflow.GatewayFlowNacosProvider;
import com.alibaba.csp.sentinel.dashboard.rule.nacos.gatewayflow.GatewayFlowNacosPublisher;
...
public class GatewayFlowRuleController {
    ...
    @Autowired
    private GatewayFlowNacosProvider provider;

    @Autowired
    private GatewayFlowNacosPublisher publisher;
...
//    }
    ...
    private boolean publishRules(String app, String ip, Integer port){
        List<GatewayFlowRuleEntity> apis = repository.findAllByMachine(MachineInfo.of(app, ip, port));
        try{
            publisher.publish(app, apis);
            return true;
        }catch (Exception e){
            logger.error("publish gateway flow rules error:", e);
            return false;
        }
    }
}
